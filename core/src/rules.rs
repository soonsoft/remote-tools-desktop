...
/// 决策链：路径围栏 → 工具 override（deny/auto）→ 会话规则 → 分类（含危险模式）。
/// 顺序即安全边界（spec §8 ①）：围栏最先且不可绕过——allowRoots 白名单外
/// 一律 path_denied 不弹窗；override "deny"/"auto" 只影响确认策略，
/// "auto" 不豁免围栏；「本会话内允许同类」两者皆不可绕过。
